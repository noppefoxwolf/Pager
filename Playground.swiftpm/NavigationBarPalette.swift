...
public final class NavigationBarPalette {
    ...
    public init?(contentView: UIView) {
        guard let paletteViewType = NSClassFromString(Strings.navigationBarPalette) as? UIView.Type else {
            return nil
        }

        let allocSelector = NSSelectorFromString(Strings.alloc)
        guard paletteViewType.responds(to: allocSelector),
              let allocatedPaletteView = paletteViewType.perform(allocSelector)?.takeUnretainedValue(),
              let paletteView = allocatedPaletteView as? UIView else {
            return nil
        }
        ...
        guard paletteView.responds(to: initSelector),
              let initializedPaletteView = paletteView.perform(
                  initSelector,
                  with: contentView
              )?.takeUnretainedValue() as? UIView else {
            return nil
        }
